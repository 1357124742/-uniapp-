<template>
	<view>
		...
		<view class="cu-card case" :class="isCard?'no-card':''" v-for="(item,index) in Buy" :key="index">
		...
		onPullDownRefresh() {
			db.collection("buy").get({
				success: res => {
					...
					{
						if(this.Buy[i].like_user.length != 0){
							for(var j =0 ;j < this.Buy[i].like_user.length; j++){
								if(this.Buy[i].like_user[j] == getApp().globalData.avatarUrl){
						...
						.get({
							success: res => { //res返回了全部数据集合
								for (var i = 0; i < res.data.length; i++) { //循环了res的总数次
									if (this.Buy[e.currentTarget.dataset.index]._id == res.data[i]._id) { //如果点赞的帖子已经被点赞过了
										var y = this.Buy[e.currentTarget.dataset.index].like_user //将该帖子的点赞用户表传给y
										...
										var x = this.Buy[e.currentTarget.dataset.index].like_number - 1
										wx.showModal({
											...
											success: res => {
												if (res.confirm) {
													db.collection("buy").doc(this.Buy[e.currentTarget.dataset.index]._id).update({
														data: {
															like_number: x,
															like_user: y,
														}
